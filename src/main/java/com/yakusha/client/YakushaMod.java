...
public class YakushaMod implements ClientModInitializer {
    ...
    public static class JumpCircle extends Feature {
        ...
        public void tick(){
            ...
            if(ct>0){for(int i=0;i<8;i++){double a=i*Math.PI/4;mc.world.addParticle(ParticleTypes.FLAME,mc.player.getX()+Math.cos(a)*0.8,mc.player.getY()+0.2,mc.player.getZ()+Math.sin(a)*0.8,0,0.02,0);}ct--;}
        }
    }
    ...
    public static class AutoFarm extends Feature {
        ...
        public void tick(){
            ...
            c++;if(c%4!=0)return;
            for(int x=-4;x<=4;x++)for(int z=-4;z<=4;z++)for(int y=-1;y<=1;y++){var p=mc.player.getBlockPos().add(x,y,z);var s=mc.world.getBlockState(p);if(s.getBlock()==Blocks.WHEAT&&s.get(CropBlock.AGE)>=7)mc.interactionManager.attackBlock(p, Direction.UP);}
        }
    }
